...
def read_linelist_file_ew(path, filelinelist):
    """
    Read the linelist file into a numpy array with the respective names
    in the columns This skips header with 2 lines
    """
    dtp = {'names': ('lambda_rest', 'EP', 'loggf', 'ele', 'atom', 'ew'),
           'formats': ('f4', 'f4', 'f4', 'a4', 'f4', 'f4')}
    data = np.loadtxt(path+filelinelist, dtype=dtp, skiprows=2)
    return data
...
def calib_loggf(path, line_data, loggf_i, loggf_f):
    ...
    while abs(ew_sun - ew_calc) > 0.05 and i < 40:
        ...
        if ew_calc - ew_sun < 0:
            ...
            b1 = c1
    # It is important to be careful with the use of the loggf vs gf in MOOG
    if i < 40:
        return loggf_out
    else:
...
def recalibrate_loggf(path, filein, fileout):
    ...
    # TODO: Same as above. This can be done in few lines with np.savetxt
