...
def create_abfind_par(path, linelist_ares):
    # TODO: Why do you guys insist on calling it abfind.par and not batch.par???
    owd = os.getcwd()
    os.chdir(path)
    os.system('cp %sAmebsa_tunned/abfind.par.base abfind.par' % get_install_dir())
...
def run_amebsa_prior(path, run_path, filename_lines,
                     teff, logg, feh, vtur, models):
    owd = os.getcwd()
    os.chdir(path)
    os.chdir(run_path)
