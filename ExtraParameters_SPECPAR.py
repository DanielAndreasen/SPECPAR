...
def correct_logg_transit_mortier(loggin, teff):
    """
    Correct a logg from ARES+MOOG accordingly with Mortier et al. 2014
    using transit data Valid for 4500 < teff < 6500
    """
...
def correct_logg_astero_mortier(loggin, teff):
    """
    Correct a logg from ARES+MOOG accordingly with Mortier et al. 2014
    using asteroseismic data Valid for 5000 < teff < 6500
    """
...
def radius_torres2010(teff, logg, feh):
    ...
    logR = bi[0]+bi[1]*X+bi[2]*X**2.+bi[3]*X**3 +\
        bi[4]*logg**2+bi[5]*logg**3+bi[6]*feh
    RT = 10**logR
    return (RT)
...
#   eai=[0.058,0.029,0.029,0.24,0.011,0.0019,0.014]
    X = math.log(teff, 10)-4.1
    logM = ai[0]+ai[1]*X+ai[2]*X**2.+ai[3]*X**3 +\
        ai[4]*logg**2+ai[5]*logg**3+ai[6]*feh
    MT = 10**logM
...
def bc_flower(teff):
    ...
    lteff = np.log10(teff)
    if (lteff < 3.7):
        bcflow = -0.190537291496456e+05+0.155144866764412e+05*lteff-0.421278819301717e+04*(lteff**2)+0.381476328422343e+03*(lteff**3)
    elif (lteff >= 3.7 and lteff < 3.9):
        bcflow = -0.370510203809015e+05+0.385672629965804e+05*lteff-0.150651486316025e+05*(lteff**2)+0.261724637119416e+04*(lteff**3)-0.170623810323864e+03*(lteff**4)
    else (lteff >= 3.9):
        bcflow = -0.370510203809015e+05+0.385672629965804e+05*lteff-0.150651486316025e+05*(lteff**2)+0.261724637119416e+04*(lteff**3)-0.170623810323864e+03*(lteff**4)
    return bcflow
...
def get_hipparcus_paralaxe(hipnumber):
    ...
    print pth
    hip, par, erpar =
    np.loadtxt('%s/hipparcus_data/Main_Cat.rdb' % pth, skiprows=2,
               usecols=(0, 6, 11), unpack=True)
    ...
    hip, v = np.loadtxt('%s/hipparcus_data/myhip.rdb' % pth,
                        skiprows=2, usecols=(1, 5), unpack=True)
    itemindex = np.where(hip == hipnumber)
    vmag = v[itemindex][0]
...
def get_url(strurl):
    ...
    for line in lines:
        if re.search("Results for", line):
            line = line.replace(".<p>Results for", "").replace("&plusmn;", ",")
            line = line.replace(":   Age=", ",").replace("Gyr , Mass=", ",")
            line = line.replace("<i>M</i>&#9737 , log<i>g</i>=", ",")
            line = line.replace("(cgs) , <i>R</i>=", ",")
            line = line.replace("<i>R</i>&#9737 ,", ",")
            line = line.replace(", (<i>B-V</i>)<sub>0</sub>=", ",")
            ...
            bv = float(line[9])
            erbv = float(line[10])
            return (age, erage, mass, ermass, logg, erlogg,
                    radius, erradius, bv, erbv)
    return -1


def get_padova_data(star, teff, erteff, feh, erfeh, V, eV, plx, eplx):
    """
    Get the padova parameters from the web interface
    http://stev.oapd.inaf.it/cgi-bin/param_1.3 using the paralaxe as a
    input - For some reason does not work in some systems...
    """
...
def get_mass_padova_iter(star, teff, erteff, logg, erlogg, feh, erfeh, mass_ini, mv, bcflow):
    """
    Get the padova parameters from the web interface
    http://stev.oapd.inaf.it/cgi-bin/param_1.3 without the knowledge of
    the paralaxe which is then determined iteratively together with the
    mass and using the logg from spectroscopy
    """
    diff_mass = 1
    massi = mass_ini
    iteration = 1
    ...
    while (diff_mass > 0.005):
        ...
        iteration += 1
        massi = masso
    return (age, erage, masso, ermasso, loggo, erloggo,
            radius, erradius, bv, erbv, parit)


def get_paralax_torres(star, teff, logg, feh, mv):
    """
    Get paralax using the mass from torres calibration and the
    bolometric correction from flower
    """
